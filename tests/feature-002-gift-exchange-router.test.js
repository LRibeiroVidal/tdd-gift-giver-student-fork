...
describe("Feature 002 - The Gift Exchange Router", () => {
	const isValidRouter = (r) => {
		expect(router).toBeInstanceOf(express.Router),
			"router should be an instance of express.Router";
	};
		...
		test("Router is mounted at `/gift-exchange`", async () => {
			const regexPaths =
				app?.router?.stack?.filter(
					(layer) =>
						layer.regexp.test("/gift-exchange") && layer.name === "router"
				) ?? [];
			expect(regexPaths).toHaveLength(1);
			expect(regexPaths[0].regexp.toString().includes("gift-exchange")).toBe(
				true
			);
		});
	});
